        ...
        <div className="p-6 flex flex-col gap-2">
            {PLATFORMS.map((p)=>{
                ...
                return (
                    ...
                    className={`flex items-center gap-3 p-3.5 rounded-xl border text-left transition-all ${isConnected ? "border-blue-200 bg-blue-500 cursor-default" :
                        "border-slate-200 bg-slate-50 hover: border-slate-300 hover:bg-slate-100 cursor-pointer"
                    } ${isConnecting && "opacity-60"}`}
                    >
                        {/* ICON */}
                        <div className="p-2">
                            <p.icon className={`size-5 ${isConnected ? "text-blue-600" : "text-slate-500"}`} />
                        </div>

                        {/* LABEL */}
                        <div className="flex-1 min-w-0">
                            <div className={`text-sm ${isConnected ? "text-blue-700" : "text-slate-800"}`}>{p.name}</div>
                        ...
                        {/* STATUS */}
                        {isConnected && <CheckCircleIcon className="size-4 yext-blue-500 shrink-0" />}
                        {isConnecting && <div className="size-4 border-2 border-blue-600 border-t-transparent rounded-full animate-spin shrink-0" />}
