        ...
        if(accounts.length === 0){
            return (
                ...
                flex flex-col items-center justify-center py-20 px-6">
                    ...
                    <p className="text-slate-700 text-lg">No accounts connected. Click "Add Account" to begin</p>
                    <p className="text-sm text-slate-400 mt-1 max-w-xs text-center">Link your first social account and let the automation begin.</p>
            ...
            return(
                <div key={index} className="group bg-white border border-slate-200 rounded-2xl p-5 flex items-center gap-4 hover:border-slate-300 transition-all">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="text-sm text-sm text-slate-500 mt-0.5">{meta.name}</div>
                    </div>
                    <div className="flex items-center gap-1.5 shrink-0">
                        {accounts.status === 'connected' ? (
                            <>
                            <CheckCircleIcon className="size-4 text-emarald-500" />
                            <span className="text-xs text-emarald-600">Connected</span>
                    ...
                    <button 
                    onClick={()=> handleDisconnect(accounts._id)}
                    title="Disconnect account"
                    className="ml-2 p-1.5 rounded-lg text-slate-300 group-hover:text-blue-500 transition-all">
