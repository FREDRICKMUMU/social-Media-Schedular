  ...
  <div className="max-w-4xl mx-auto space-y-12 pb-20 animate-in fade-in duration-700">
    ...
    <div className="space-y-6 text-center mt-20">
      ...
      <div className="relative group mt-12">
        <textarea 
          className="w-full px-6 py-6 bg-white border border-slate-300 rounded-xl text-slate-900 placeholder-slate-400
          outline-none focus:border-slate-400 transition resize-none h-40"
        ...
        <div className="absolute bottom-4 right-2.5 flex items-center gap-3 text-sm">
          <button 
            ...
            className="flex items-center gap-3 bg-blue-50 py-2 px-3 rounded-lg"
          >
            <span>AI Image</span>
            <div className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full
              transition-colors duration-200 ease-in-out focus:outline-none ${generateImage ? "bg-blue-500" : "bg-slate-200"}`}>
      ...
      <div className="flex flex-wrap justify-center gap-2">
        {tones.map((t) => (
          <button 
            key={t} 
            onClick={() => setTone(t)} 
            className={`px-4 py-1.5 rounded-full text-sm transition-all border ${tone === t ? "bg-blue-500 border-blue-500 text-white" :
    ...
    <div className="space-y-6 pt-12 border-t border-slate-100">
      <div className="flex items-center justify-between text-slate-600">
        ...
        <span className="text-sm text-slate-500 bg-slate-50 px-2 py-1 rounded">{generations.length} total</span>
      </div>  
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {generations.map((gen) => (
          <div 
            key={gen._id}
            className="group bg-white rounded-2xl border border-slate-100 p-5 hover:border-blue-200
          ...
          >
            <div className="flex flex-col h-full space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-blue-500 bg-blue-50 px-2 py-0.5 rounded-md">{gen.tone}</span>
              </div>
              ...
              {gen.mediaUrl && (
                <div className="rounded-xl overflow-hidden border border-slate-50 bg-slate-50">
                  <img src={gen.mediaUrl} alt="Generated" className="w-full aspect-video object-cover opacity-90
              ...
              <div className="flex items-center gap-2 pt-2">
                <button 
                  ...
                  className="flex-1 bg-slate-100 hover:bg-blue-500 hover:text-white text-slate-600
      ...
      <div className="fixed inset-0 min-h-screen z-50 flex items-center justify-center p-4
        ...
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl border border-slate-100 overflow-hidden
          ...
          <div className="flex items-center justify-between px-8 py-4 border-b border-slate-100 bg-slate-50/30">
            <h3 className="text-slate-900 font-semibold">Schedule Generation</h3>
            <button
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-4">
            ...
            <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 space-y-4">
              ...
              {activeScheduler.mediaUrl && (
                <img src={activeScheduler.mediaUrl} alt="preview" 
          ...
          <div className="p-8 bg-slate-50/50 border-t border-slate-50 space-y-8">
            ...
            <div className="space-y-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {PLATFORMS.map((p) => {
                    ...
                    return (
                      <button 
                        key={p.id}
                        className={`p-2.5 rounded-md border text-xs transition-all ${active ?
                          "bg-blue-500 text-white border-blue-500" : "bg-white border-slate-200 text-slate-400 hover:border-slate-300"}`}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="relative">
                  ...
                  <input 
                    type="date" 
                    className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-md
                    text-slate-900 text-sm focus:outline-none transition-all" 
                    value={scheduledDate}
            ...
            <button 
              ...
              className="w-full flex items-center justify-center gap-2 py-3 rounded-md bg-slate-800
              text-white hover:bg-red-500 transition-all"
            >
