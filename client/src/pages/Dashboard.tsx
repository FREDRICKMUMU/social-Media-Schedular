  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-slate-500 text-sm mt-0.5">Here's your social media snapshot for today.</p>
      </div>

      {/* STAT CARDS */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {statCards.map((card) => (
          <div key={card.label} className="bg-white hover:bg-blue-50 relative border border-slate-200 rounded-2xl p-5 hover:border-blue-200 transition-all">
            <div className="flex items-center justify-between mb-4">
              <div className="text-3xl font-medium text-slate-800 tabular-nums">{card.value}</div>
              <div className="text-xs absolute right-4 top-4 text-green-500 flex items-center gap-1">
        ...
        <div className="px-6 py-8">
          {activities.length === 0 ? (
            <div className="flex flex-col items-center justify-center text-center py-8">
              ...
              <p className="text-slate-500 font-medium">No activity to display</p>
              <p className="text-slate-400 text-sm mt-1">Link your accounts and start scheduling to see activity.</p>
            </div>
          ) : (
