...
const Scheduler = () => {
 ...
 const [scheduledTime, setScheduledTime] = useState("");
   ...
   const sheduled = posts.filter((p)=> p.status === "scheduled")
      ...
      const togglePlatform = (id: string)=> setSelectedPlatforms(prev)=> (preview.includes(id) ? prev.filter((p)=> p !== id) : 
    [...prev, id])
        ...
        const handleSchedule = async (e: React.FormEvent) => {
          ...
          setTimeout(()=>{
            setLoading(false)
            setPosts(prev)=> [...preview, dummyPostsData[0]]
          }, 1000)
        }
    ...
    <div className="flex flex-col lg:flex-row gap-6 h-full">
      {/* COMPOSE PANEL */}
        <div className="w-full lg:w-[460px] shrink-0">
          <div className="bg-hite rounded-2xl border border-slate-200 p-6">
            ...
            <form className="space-y-5" onSubmit={handleSchedule}>
                ...
                <div className="flex flex-rap gap-3">
                  {PLATFORMS.map((p)=>{
                    ...
                    return(
                      ...
                      className={`flex items-center gap-1.5 p-3 rounded-md border transition-all duration-150 ${active ? "bg-blue-50
                        border-blue-300 text-red-500 scale-103" : "border-slate-200 text-slate-500 hover: border-slate-300"}`}>
              ...
              className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-900 text-sm
              placeholder-slate-400 outline-none resize-none"
              value={content} onChange={(e)=>setContent(e.target.value)}
              />

              <div className={`text-right text-xs mt-1 font-medium ${content.length > 270 ? "text-blue-500" : "text-slate-400"}`}>
              {content.length}/280</div>
              </div>
              </div>
              ...
              className="absolute top-2 right-2 size-7 bg-slate-900/60 hover:bg-slate-900/80 text-white rounded-full flex items-center
              ...
              border-2 border-dashed border-slate-200 rounded-xl cursor-pointer hover:border-blue-300 hover:bg-blue-50/30 transition-all group">
              <span className="text-sm text-slate-500 group-hover:text-blue-600 transition-colors">Click to upload image or video</span>
              ...
              <CalenderIcon className="size-4 absolute left-3 top-1/2 -transalte-y-1/2 text-slate-400 pointer-events-none" />
              <input type="date" required className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border 
              border-slate-200 rounded-lg text-slate-900 text-sm outline-none"
              ...
              <label className="block text-xs text-slate-500 uppercase mb-2">Date</label>
              <div className="relative">
              <ClockIcon className="size-4 absolute left-3 top-1/2 -transalte-y-1/2 text-slate-400 pointer-events-none" />

              <input type="time" required className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border 
              border-slate-200 rounded-lg text-slate-900 text-sm outline-none"
              ...
              bg-blue-500 hover:bg-blue-600 transition-all text-white rounded-lg
              ">
              {loading ? (
              <>
      ...
      {scheduled.length === 0 ? (
       ...
       <div className="flex items-center gap-2">
        ...
        {post.mediaType && <span className="text-xs bg-slate-100 text-slate-600 border
        border-slate-200 px-1.5 py-0.5 font-semibold capitalize rounded-md">{post.mediaType}</span>}
      ...
      {scheduled.length === 0 ? (
       ...
       <div className="flex items-center gap-2">
        ...
        <span className="text-xs bg-emarald-50 text-emarald-700 border border-emarald-100 px-2 py-0.5 rounded-full">Published</span>
